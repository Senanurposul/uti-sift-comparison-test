...
def build_response_sift_comparison_test(context):
    ...
    # Visualize=False ise ya da InputVisualization1/2 bağlı
    # değilse üçü de None kalır.
    output_visualization_1 = OutputVisualization1(
        value=getattr(context, "output_visualization_1", None)
    )

    output_visualization_2 = OutputVisualization2(
        value=getattr(context, "output_visualization_2", None)
    )

    output_visualization_matches = OutputVisualizationMatches(
        value=getattr(context, "output_visualization_matches", None)
    )
